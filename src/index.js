...
const VERSION='2.2.0-csgt-persistent',PORT=Number(process.env.PORT||3000),EXTERNAL_URL=process.env.RENDER_EXTERNAL_URL||process.env.WEBHOOK_BASE_URL||'',WEBHOOK_PATH='/telegram';
const bot=new Telegraf(TOKEN),subscriptions=new Map();
...
async function handleLookup(ctx,raw,type='1'){const p=normalizePlate(raw);if(!isLikelyPlate(p))return ctx.reply('Biển số chưa đúng định dạng. Ví dụ: 43A40281');const w=await ctx.reply(`🔎 [${VERSION}] Đang tra cứu ${p} trực tiếp CSGT. Bot sẽ tiếp tục chờ kết quả...`);try{const r=await lookupMultiSource(p,type);await ctx.telegram.editMessageText(ctx.chat.id,w.message_id,undefined,resultText(p,r))}catch(e){await ctx.telegram.editMessageText(ctx.chat.id,w.message_id,undefined,`❌ [${VERSION}] Chưa lấy được kết quả ${p}.\n\n${String(e.message||e).slice(0,1000)}`)}}
bot.start(ctx=>ctx.reply(`🚦 Tra cứu phạt nguội Việt Nam\n⚙️ ${VERSION}\nNguồn: CSGT Bộ Công An. Bot ưu tiên lấy kết quả, không dừng ở timeout 30 giây.`));
...
const server=app.listen(PORT,async()=>{try{if(EXTERNAL_URL){const u=`${EXTERNAL_URL.replace(/\/$/,'')}${WEBHOOK_PATH}`;await bot.telegram.setWebhook(u,{drop_pending_updates:false});console.log('webhook',u)}else await bot.launch({dropPendingUpdates:false})}catch(e){console.error(e.message)}});
